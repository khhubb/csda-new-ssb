//### validation functions called with a caster-specific value: for now, parameter left as an int
...
bool CCastStringValidator::IsAuditChangeIndicatorOk(CCSOrder* > :const_iterator io) {
	bool isOk = true;
	if ((*io)->ChangeMarked()) {
...
bool CCastStringValidator::Validate340080(int strandNum)
{
	...
	if (strand.size() == 0) // then there's nothing to check
		return true;
	...
	for (vector<CCSOrder*>::const_iterator io = strand.begin(); io != strand.end(); ++io) {
		...
		isOk =
			...
			IsSteelLengthOk(io, caster) &&           // block  5
			IsPlannedLengthOk(io) &&                 // block  6
			IsHeatPositionOfCombi3Ok(io, caster) &&  // block  7
			IsSteelLengthComplianceOk(io, caster) && // block  8
			IsStrandNumOk(io) &&                     // block  9
			IsExposureCodeOk(io, caster) &&          // block 10
			IsTekKotePositionalOrderOk(io) &&        // block 11
			IsWidthChangeOk(io) &&                   // block 12
			Is80inSpreadSqueezeOk(io) &&             // block 13
			IsSlitTypeCodeOk(io) &&                  // block 14
			IsWidthJumppingOk(io) &&                 // block 15
			IsStockReasonOk(io) &&                   // block 16
			IsSlitReasonOk(io) &&                    // block 17
			Is2BOFInwardWidthChangeOk(io, caster) && // block 18
			IsAuditChangeIndicatorOk(io);            // block 19
		  ...
		  /** ###
		  {
			  ...
			  // Note: K. Hubbard 12-11-03; Add New Fatal here by checking width (*io)->ProvSlabWidthMin & Max
					  ...
					  // audit non Transition Provided Steel widths ##### 4
					  /** ###
					  {
						  Width width = (*io)->SlabWidth();
						  char code   = (*io)->SlitTypeCode();
			  ...
			  //			Width provWidthMax = (*io)->ProvSlabWidthMax();
						  ...
						  //					ADD_ERR(CCastStringHeatValidnError::FATAL);
									  ...
									  ## **/
									  /**###
									//  check heat position for 3 combi startup slabs ##### 7
									{
										if ( caster == 3 ) { //### caster-specific
							...
							//				}           comm out 7-01-04 k. hubbard // spec check
									...
									/** ##
										// audit steel length compliance with steelshop limitations ##### 8
										{
											Length len = (*io)->SlabLength();
								...
								//			}
									...
									/** ##
										...
										// Audit Tek and Kote positional planned orders at beginning of cast violations.  Added 03-06-06 k. hubbard per P. Velasco ##### 11
									/** ##
										{
								...
								//				ADD_ERR(CCastStringHeatValidnError::FATAL);   // Changed to warning per new 1st cast piece cropping plans. 02/20/07 k. hubbard
										...
										/** ##
											// audit width change.  Radicals are ok here 12-11-03 k. hubbard ##### 12
											{
										...
										**/
										...
										////			prevWidth = (*io)->SlabWidth();  //Important !!!! repositioned below width jump audit to work inside this For loop and share between width audits 11-22-05 k. hubbard
											...
											/** ##
												// Audit Transition 80" Slab Spread Squeeze violations against Provided Cast Widths ##### 13
												if ( (*io)->Order() != 0 )
												{
													Width width = (*io)->SlabWidth();
										//			Width ProvWidthMax = (*io)->ProvSlabWidthMax();
										...
										//					(NarrowestWidthPoint > ProvWidthMax || NarrowestWidthPoint < ProvWidthMin))  //Comm out switched per P. Fronczek maint. 4-18-07 k. hubbard
												...
												**/
												...
												// Audit Side Slits Minimum Allowance change.  Added 5-24-06 k. hubbard per P. Velasco ##### 14
											/** ##
												{
												  char code = (*io)->SlitTypeCode();
												  bool isStock = ( (*io)->FpOrderNum().Left(7) == "9999999" );
											...
											**/
											...
											// Audit Width Jumps change.  Added 11-29-05 k. hubbard per P. Velasco ##### 15
										/** ##
											{
									...
									//				prevWidth = (*io)->SlabWidth();
											...
											// Audit Width Jumps change end 11-29-05 k. hubbard ##### 16
											**/
										...
										**/
										// Audit Slit Reason Code  added audit for slit reason 2-26-10 k. hubbard ##### 17
									/**
										{
											char code = (*io)->SlitTypeCode();
											CString slitFaclReason = (*io)->SlitReason();
									...
									**/
									// Audit 2BOF inward width change designs.  1-28-10 k. hubbard ##### 18
								/**
								{
