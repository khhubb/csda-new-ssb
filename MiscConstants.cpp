...
Length CMiscConstants::m_casterSlabLengthMaxes[4] = { 0, 452, 452, 431 };  // Changed 1SC and 2SC to 452" max plan length, exceeding these limits writes 'Fatal' in CastStringValidator,cpp k. hubbard 03-31-16 maint.
...
CMiscConstants::CMiscConstants()
{
	...
	m_turnaroundTimes[1] = CTimeSpan(0,0,0,0);
	m_turnaroundTimes[2] = CTimeSpan(0,0,75,0);
	m_turnaroundTimes[3] = CTimeSpan(0,0,90,0);
...
void CMiscConstants::Init(CString loginName_)
{
	...
	try 
	{
		...
		m_minHeatSizes[1] = rs.m_Caster1HeatSizeMin;
		m_aimHeatSizes[1] = rs.m_Caster1HeatSizeAim;
		m_maxHeatSizes[1] = rs.m_Caster1HeatSizeMax;
		m_minHeatSizes[2] = rs.m_Caster2HeatSizeMin;
		m_aimHeatSizes[2] = rs.m_Caster2HeatSizeAim;
		m_maxHeatSizes[2] = rs.m_Caster2HeatSizeMax;
		m_minHeatSizes[3] = rs.m_Caster3HeatSizeMin;
		m_aimHeatSizes[3] = rs.m_Caster3HeatSizeAim;
		m_maxHeatSizes[3] = rs.m_Caster3HeatSizeMax;
...
void CMiscConstants::Save()
{
	...
	try 
	{
		...
		rs.m_Caster1HeatSizeMin = m_minHeatSizes[1];
		rs.m_Caster1HeatSizeAim = m_aimHeatSizes[1];
		rs.m_Caster1HeatSizeMax = m_maxHeatSizes[1];
		rs.m_Caster2HeatSizeMin = m_minHeatSizes[2];
		rs.m_Caster2HeatSizeAim = m_aimHeatSizes[2];
		rs.m_Caster2HeatSizeMax = m_maxHeatSizes[2];
		rs.m_Caster3HeatSizeMin = m_minHeatSizes[3];
		rs.m_Caster3HeatSizeAim = m_aimHeatSizes[3];
		rs.m_Caster3HeatSizeMax = m_maxHeatSizes[3];
