...
BEGIN_MESSAGE_MAP(CCastStringEditorScenMgrController, MvcController)
	...
	ON_BN_CLICKED(IDC_RB_CASTER_4, OnRbCaster4)
	ON_BN_CLICKED(IDC_RB_CASTER_5, OnRbCaster5)
...
void CCastStringEditorTextViewport::SetStrandOrderListHeaders()
{
	...
	static const char* strExpDisp   = "ExpDisp";
	...
	static const int widthExpDisp   = CompWidth3( 15, strExpDisp,   "999"           );
	...
	DefCol(LVCFMT_RIGHT,  strExpDisp,   widthExpDisp   ); // 11
	DefCol(LVCFMT_RIGHT,  strPcs,	    widthPcs       ); // 12
	DefCol(LVCFMT_RIGHT,  strTons,      widthTons      ); // 13
	DefCol(LVCFMT_RIGHT,  strPrio,      widthPrio      ); // 14
	DefCol(LVCFMT_RIGHT,  strStartTime,	widthStartTime ); // 15
	DefCol(LVCFMT_RIGHT,  strLength,    widthLength    ); // 16
//	DefCol(LVCFMT_RIGHT,  strHrWk,      widthHrWk      ); // 16
	DefCol(LVCFMT_CENTER, strHrWk,      widthHrWk      ); // 17
	DefCol(LVCFMT_RIGHT,  strCI,        widthCI        ); // 19
	DefCol(LVCFMT_CENTER, str80Type,	width80Type	   ); // 19
	DefCol(LVCFMT_LEFT,	  strCust,		widthCust	   ); // 20
	DefCol(LVCFMT_RIGHT,  strC,         widthC         ); // 21
	DefCol(LVCFMT_RIGHT,  strMn,        widthMn        ); // 22
	DefCol(LVCFMT_RIGHT,  strSignifEl,  widthSignifEl  ); // 23
}
...
//	listCtrl.SetItemText(itemNum,  3, ostr.str()); increased column count by 1 to make room for new Hotroll unitname column 4-28-10 k. hubbard
	...
	if ( pCSOrder->Order() == 0 ) {
		...
		listCtrl.SetItemText(itemNum,20,"?");
	}
	else {
...
//		listCtrl.SetItemText(itemNum,2,_itoa(pCSOrder->Order()->HotrollUnitName(),buf,10));
		...
		listCtrl.SetItemText(itemNum,18,_itoa(pCSOrder->Order()->CICode(),buf,10));
		...
		listCtrl.SetItemText(itemNum,17,_itoa(pCSOrder->Order()->HrYrWk().Week(),buf,10));
		...
		listCtrl.SetItemText(itemNum,15,buf);

		strcpy(buf,LPCTSTR(pCSOrder->Order()->CustName().Left(10)));
		listCtrl.SetItemText(itemNum,20,buf);

		listCtrl.SetItemText(itemNum,19,pCSOrder->Order()->MegaLuCd());
...
//	listCtrl.SetItemText( itemNum,  5, _ltoa(long(pCSOrder->SlabWidth()),  buf, 10));  // 9-14-07 k.hubbard original comm. out b4 testing above.
	...
	listCtrl.SetItemText(itemNum,  11, _itoa(pCSOrder->ExpandedDispCode(), buf, 10));
	listCtrl.SetItemText( itemNum, 12, _itoa(pCSOrder->NumPieces(),        buf, 10));
	listCtrl.SetItemText( itemNum, 16, _ltoa(long(pCSOrder->SlabLength()), buf, 10));
	listCtrl.SetItemText( itemNum, 13, _ltoa(long(pCSOrder->Tons()),       buf, 10));
	...
	listCtrl.SetItemText( itemNum,23, LPCTSTR(strChem) );
